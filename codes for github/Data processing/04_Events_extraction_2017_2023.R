...
################################################################################ nc/tif folder
ffmc <- mixedsort(dir('D:\\000_collections\\020_Chapter2\\021_Data\\0213_FireWeather79-16\\Hourly_FFMC_NA', full.names = T,pattern = '(.nc)$'))
temp <- mixedsort(dir('D:\\000_collections\\020_Chapter2\\021_Data\\0213_FireWeather79-16\\Hourly_Temp_NA', full.names = T,pattern = '(.nc)$'))
rh <- mixedsort(dir('D:\\000_collections\\020_Chapter2\\021_Data\\0213_FireWeather79-16\\Hourly_RH_NA', full.names = T,pattern = '(.nc)$'))
prec <- mixedsort(dir('D:\\000_collections\\020_Chapter2\\021_Data\\0213_FireWeather79-16\\Hourly_Prec_NA', full.names = T,pattern = '(.nc)$'))
winds <- mixedsort(dir('D:\\000_collections\\020_Chapter2\\021_Data\\0213_FireWeather79-16\\Hourly_WS_NA', full.names = T,pattern = '(.nc)$'))
# vpd <- mixedsort(dir('D:\\000_collections\\020_Chapter2\\021_Data\\0213_FireWeather79-16\\Hourly_FFMC_NA', full.names = T,pattern = '(.nc)$'))
# emc <- mixedsort(dir("D:\\000_collections\\020_Chapter2\\021_Data\\0213_FireWeather79-16\\Hourly_FFMC_NA", full.names = T,pattern = '(.nc)$'))
isi <- mixedsort(dir('D:\\000_collections\\020_Chapter2\\021_Data\\0213_FireWeather79-16\\Hourly_ISI_NA', full.names = T,pattern = '(.nc)$'))
...
f<-function(i){
  ...
  if (dim(GOES_fireinfo)[1] > 0){   
    ...
    if (year.indi==2017) {  startseq=456 } else if (year.indi==2018) {startseq=468 } else if (year.indi==2019) {
      startseq=480 }else if (year.indi==2020) { startseq=492 }else if (year.indi==2021) {startseq=504}else if (year.indi==2022) {
        startseq=516}else if (year.indi==2023) {startseq=528}
    ...
    if (startmonth+startseq==endmonth+startseq) {
      ...
      out.val <- cbind(out.val.ffmc,out.val.temp,out.val.rh,out.val.prec,out.val.winds,out.val.isi)
      out.val <- data.frame(out.val)
      out.val$biome <- biomeid
      
      rm(var.ffmc.nc,var.temp.nc,var.rh.nc,var.prec.nc,var.winds.nc,var.isi.nc )
      rm(ext.val.temp,ext.val.rh,ext.val.prec,ext.val.winds,ext.val.isi)
      rm( out.val.ffmc,out.val.temp,out.val.rh,out.val.prec,out.val.winds,out.val.isi)
      ...
      for (l in seq((startmonth+startseq),(endmonth+startseq),1)) {
        ...
        if (l==startmonth+startseq) {
          ...
          out.val.2 <- cbind(out.val.ffmc.2,out.val.temp.2,out.val.rh.2,out.val.prec.2,out.val.winds.2,out.val.isi.2)
          ...
          out.val.3 <- cbind(out.val.ffmc.3,out.val.temp.3,out.val.rh.3,out.val.prec.3,out.val.winds.3,out.val.isi.3)
    ...
    out.name <- paste('D:\\000_collections\\222_hourly fire potential\\023_outputs\\NAfires_extrect_2017_2023\\',year.indi,'_',shp.indi$id_yr,'_',as.integer( shp.indi$id_glb),'_',shp.indi$Country,'_',
...
registerDoSNOW(cl)
pb <- txtProgressBar(min=1400, max=length(NAfire17to23), style=3)#length(NAfire17to23)
progress <- function(n) setTxtProgressBar(pb, n)
opts <- list(progress=progress)

start_time <- Sys.time()
nbac_csvwrite <- foreach(i=1400:length(NAfire17to23), .options.snow = opts) %dopar%  {  
  tryCatch(
    f(i),
    error = function(e) {
